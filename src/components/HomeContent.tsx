...
export type BrandWithCountry = Brand & { country: string };

interface HomeContentProps {
  /** When provided (e.g. from brand route), pre-select country and open this brand's modal. */
  initialCountry?: string;
  initialBrand?: BrandWithCountry | null;
}

export default function HomeContent({ initialCountry, initialBrand }: HomeContentProps) {
  const { t } = useLocale();
  const searchParams = useSearchParams();
  const [showOnboarding, setShowOnboarding] = useState(!initialBrand);
  const [activeCountry, setActiveCountry] = useState(initialCountry ?? "all");
  const [activeCat, setActiveCat] = useState("all");
  const [selectedBrand, setSelectedBrand] = useState<BrandWithCountry | null>(initialBrand ?? null);
  const [search, setSearch] = useState("");

  // Brand route: initial state already set; no searchParams override.
  // Home page: deep-link from ?country= and ?brand=.
  useEffect(() => {
    if (initialBrand != null) {
      setShowOnboarding(false);
      setActiveCountry(initialBrand.country);
  ...
  useEffect(() => {
    ...
    if (!brandSlug) return;
    const country = searchParams.get("country");
    if (!country || !VALID_COUNTRIES.has(country)) return;
    const brand = getBrandByCountrySlug(country, brandSlug);
